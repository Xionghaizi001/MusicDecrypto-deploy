...
Common settings:
  ...
  NODE_BIN=/path/to/node optional
...
run_frontend_build_shell() {
  ...
  if [ -n "$NODE_BIN" ]; then
    ...
    command="export PATH=$(shell_quote "$node_dir"):\$PATH; export NODE=$(shell_quote "$NODE_BIN"); $command"
  fi
...
resolve_runtime_config() {
  ...
  if ! was_provided NODE_BIN; then
    NODE_BIN="$(web_env_value MUSICDECRYPTO_MANAGE_NODE_BIN || true)"
    NODE_BIN="${NODE_BIN:-$(env_value MUSICDECRYPTO_MANAGE_NODE_BIN || true)}"
  fi
